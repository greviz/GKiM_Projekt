...
int main(int argc, char** argv)
{
	...
	kolor = 0; //ustawienie defaultowych wartoœci dla kompresora, gdyby u¿ytkownik nie wybra³ w³asnych parametrów
	paleta = 1;
	dithering = 0;
	counter = 0;
		...
		while (SDL_PollEvent(&event))
		{
			...
			else if (SDL_MOUSEBUTTONDOWN == event.type)
			{
				...
				if (counter >= 1)
				{
					...
					if (event.button.y > height && event.button.y < height + 60) //przycisk do konwersji pojawi siê dopiero po wybraniu jakiegoœ parametru
					{
						saveBitmapToFile();
						convertMsgBox();
					}
			...
			else if (SDL_KEYDOWN == event.type)
			{
				...
				if (event.key.keysym.sym == SDLK_1) //przycisk aktywuj¹cy funkcjê dekompresuj¹c¹ w kompresorze ¿eby sprawdziæ czy kompresja zakoñczy³a siê powodzeniem
				{
					readBitmapFromFile();
				}
			}
...
int initSdl()
{
	...
	screen = SDL_GetWindowSurface(window); //inicjalizacja wszystkich obiektów potrzebnych do wyœwietlania obrazów
	renderer = SDL_CreateRenderer(window, -1, 0);

	initButtons(); //inicjalizacja przycisków
...
vector<int> Bit6Color() //funkcja do przetworzenia kolorów na obrazku na 6bit
{
	vector<int> p;
...
vector<int> Bit6GreyScale() //funkcja do przetworzenia kolorów na obrazku na 64 stopniowa skale szarosci
{
	vector<int> p;
	double wspolczynnik;
	SDL_Color pomocniczy;
